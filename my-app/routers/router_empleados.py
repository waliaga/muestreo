...
PATH_URL_E = "public/empleados"

@app.route('/registrar-empleado', methods=['GET'])
def viewFormEmpleado():
    if 'conectado' in session:
        return render_template(f'{PATH_URL_E}/form_empleado.html')
    else:
...
def lista_empleados():
    if 'conectado' in session:
        return render_template(f'{PATH_URL_E}/lista_empleados.html', empleados=sql_lista_empleadosBD())
    else:
        flash('primero debes iniciar sesión.', 'error')
        return redirect(url_for('inicio'))
...
def detalleEmpleado(idEmpleado=None):
    if 'conectado' in session:
        ...
        if idEmpleado is None:
            ...
            return render_template(f'{PATH_URL_E}/detalles_empleado.html', detalle_empleado=detalle_empleado)
    else:
        flash('Primero debes iniciar sesión.', 'error')
        return redirect(url_for('inicio'))
...
def viewBuscarEmpleadoBD():
    ...
    if resultadoBusqueda:
        return render_template(f'{PATH_URL_E}/resultado_busqueda_empleado.html', dataBusqueda=resultadoBusqueda)
    else:
...
def viewEditarEmpleado(id):
    if 'conectado' in session:
        ...
        if respuestaEmpleado:
            return render_template(f'{PATH_URL_E}/form_empleado_update.html', respuestaEmpleado=respuestaEmpleado)
        else:
