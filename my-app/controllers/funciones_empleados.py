            ...
            with conexion_PostgreSQLdb.cursor(cursor_factory=DictCursor) as cursor:
                ...
                sql = """
                    INSERT INTO tbl_empleados 
                        (nombre_empleado, apellido_empleado, sexo_empleado, telefono_empleado, email_empleado, profesion_empleado, foto_empleado, salario_empleado) 
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                """

                # Creando una tupla con los valores del INSERT
                valores = (
                    dataForm['nombre_empleado'], dataForm['apellido_empleado'], dataForm['sexo_empleado'],
                    dataForm['telefono_empleado'], dataForm['email_empleado'], dataForm['profesion_empleado'],
            ...
            with conexion_PostgreSQLdb.cursor(cursor_factory=DictCursor) as cursor:
                querySQL = """
                    SELECT 
                        ...
                        e.telefono_empleado,
                        e.profesion_empleado,
                        TO_CHAR(e.fecha_registro, 'DD Mon YYYY HH12:MI AM') AS fecha_registro,
                        CASE
                            WHEN e.sexo_empleado = 1 THEN 'Masculino'
                            ELSE 'Femenino'
                        END AS sexo_empleado
...
def buscarEmpleadoBD(searchxxx):
    try:
        with connectionBD() as conexion_PostgreSQLdb:
            with conexion_PostgreSQLdb.cursor(cursor_factory=DictCursor) as cursor:
                querySQL = """
                    SELECT 
                        ...
                        END AS sexo_empleado
                    FROM tbl_empleados AS e
                    WHERE e.nombre_empleado ILIKE %s 
                    ORDER BY e.id_empleado DESC
                """
